...
def convert_xml_to_json(input_file_contents):
    data_dict = xmltodict.parse(input_file_contents)
    converted_contents = json.dumps(data_dict, indent=4)
...
def main():
    ...
    try:
        converted_data = convert_xml_to_json(input_key_data)

        message = "xml to json file conversion is SUCCESSFUL"
        splunk.log_message({'Status': 'success', 'InputFileName': input_file, 'Message': message}, get_run_id())

    except xmltodict.expat.ExpatError as e:
        message = f"xml to json file conversion failed due to expat error: {str(e)}"
        ...
        message = f"xml to json file conversion failed due to other error: {str(e)}"
        splunk.log_message({'Status': 'failed', 'InputFileName': input_file, 'Message': message}, get_run_id())
        raise Exception(message)
    ...
    try:
        ...
        message = "converted json file write operation to s3 bucket is SUCCESSFUL"
        splunk.log_message({'Status': 'success', 'InputFileName': input_file, 'Message': message}, get_run_id())
