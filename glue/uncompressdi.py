...
class Extract:
    def __init__(self, bucket, key,inputdestination,outputdestination):
        self.bucket = bucket
        self.key = key
        self.inputdestination = inputdestination
                    ...
                    if tar_resource.isfile():
                        with tar.extractfile(tar_resource) as inner_file:
                            while True:
                                ...
                                if self.subpath:
                                    s3.put_object(Body=chunk, Bucket=self.bucket, Key=self.outputdestination+tar_resource.name)
                                    #s3.put_object(Body=chunk, Bucket=self.bucket, Key=self.outputdestination+ self.key.split("/")[-1].split(".tar")[0])
                                    count += 1
                                else:
                                    s3.put_object(Body=chunk, Bucket=self.bucket, Key=self.outputdestination+tar_resource.name)
                                    count += 1
            splunk.log_message({'Status': 'success', 'InputFileName': self.key, 'Message': f'{self.key} Tar file is uncompressed SUCCESSFULLY'},  get_run_id())
    ...
    def extract_gz_file(self):
        try:
            ...
            print(fo)
            if self.subpath:
                s3.upload_fileobj(Fileobj=fo, Bucket=self.bucket, Key=self.outputdestination+fo.read(1).decode('utf-8'))
                #s3.upload_fileobj(Fileobj=fo, Bucket=self.bucket, Key=self.outputdestination+self.key.split("/")[-1].split(".gz")[0])
            else:
                s3.upload_fileobj(Fileobj=fo, Bucket=self.bucket, Key=self.outputdestination+fo.read(1).decode('utf-8'))
                #s3.upload_fileobj(Fileobj=fo, Bucket=self.bucket, Key=self.outputdestination+self.key.split("/")[-1].split(".gz")[0])
            count=1
            splunk.log_message({'Status': 'success', 'InputFileName': self.key, 'Message': f'{self.key} GZIP file is uncompressed SUCCESSFULLY'},  get_run_id())
        except Exception as e:
    ...
    def extract_zip_file(self):
        try:
            ...
            for file in zipped.namelist():
                ...
                if self.subpath:
                    s3.upload_fileobj(Fileobj=zipped.open(file), Bucket=self.bucket, Key=self.outputdestination+ final_file_path)
                else:
                    s3.upload_fileobj(Fileobj=zipped.open(file),Bucket=self.bucket, Key=self.outputdestination+ final_file_path)
                #count = count + 1
            #s3.upload_fileobj(Fileobj=zipped.open(file),Bucket=self.bucket, Key=self.outputdestination+ self.key)
            splunk.log_message({'Status': 'success', 'InputFileName': self.key, 'Message': f'{self.key} ZIP file is uncompressed SUCCESSFULLY'},  get_run_id())    
        except Exception as e:
    ...
    def extract_7z_file(self):
        try:
            ...
            for file in os.listdir(r"\tmp"):
                if self.subpath:
                    s3.upload_file(os.path.join(r"\tmp",file),Bucket=self.bucket, Key=self.outputdestination+file)
                else:
                    s3.upload_file(os.path.join(r"\tmp",file),Bucket=self.bucket, Key=self.outputdestination+file)
                print("s3 upload")
                count = count + 1
                print("count")
            result = splunk.log_message({'Status': 'success', 'InputFileName': self.key, 'Message': f'{self.key} 7Z file is uncompressed SUCCESSFULLY'},  get_run_id()) 
            print(result)
        except Exception as e:
...
try:
    ...
    iparm = getResolvedOptions(sys.argv, ['bucket', 'key','trackrecords','inputdestination','outputdestination'])
    
    bucket=iparm['bucket']
    key=iparm['key']
    trackrecsfn=iparm['trackrecords']
    inputdestination=iparm['inputdestination']
    outputdestination=iparm['outputdestination']
    result = splunk.log_message({'Status': 'success', 'Message': f'uncompression process for {key} started SUCCESSFULLY'}, get_run_id()) 
    copy_source={'Bucket':bucket,'Key':key}
    s3.copy_object(CopySource=copy_source,Bucket=bucket, Key=inputdestination+ (key.split('/')[-1]))
    extract_obj = Extract(bucket, key,inputdestination,outputdestination)
