...
def get_configs():
    ...
    args = getResolvedOptions(sys.argv, [
        'client_name', 'data_base', 'schemas', 'ssmdbkey'
    ])

    # Optional param handling
    exclude_tables = None
    if any(arg.startswith('--exclude_tables') for arg in sys.argv):
        try:
            extra_args = getResolvedOptions(sys.argv, ['exclude_tables'])
            exclude_tables = extra_args['exclude_tables']
    ...
    args['exclude_tables'] = exclude_tables
    return args
...
def truncate_tables(ssmdbkey, client_name, data_base, schemas, exclude_tables=None):
    ...
    try:
        ...
        if result:
            ...
            if "success" in return_text.lower():
                ...
                splunk.log_message({'Status': 'SUCCESS', 'Message': message}, get_run_id())
            elif "failed" in return_text.lower():
                splunk.log_message({'Status': 'Warning', 'Message': return_text}, get_run_id())
                message = f'{program_name} executed truncate_schema failed!'
                splunk.log_message({'Status': 'Warning', 'Message': message}, get_run_id())
                raise ValueError(f"{message} {return_text}")
            else:
                splunk.log_message({'Status': 'ERROR', 'Message': return_text}, get_run_id())
                message = f'{program_name} executed truncate_schema has an error!'
                splunk.log_message({'Status': 'ERROR', 'Message': message}, get_run_id())
                raise ValueError(f"{message} {return_text}")
