-- FUNCTION: public.extract_summary_details_from_semi_bal_for_asset_alloc(text, text, text, text, text)

-- DROP FUNCTION IF EXISTS public.extract_summary_details_from_semi_bal_for_asset_alloc(text, text, text, text, text);

CREATE OR REPLACE FUNCTION public.extract_summary_details_from_semi_bal_for_asset_alloc(
        ...
        SUM(
            CASE
                WHEN ba.balanceamounttype IN (
                    ...
                    'TotalCertificatesOfDeposits','TotalUnitInvestmentTrust','TotalMutualFunds','TotalRealAssets',
                    'TotalMixedAssets')
                        ...
                        jsonb_build_object(
                                ...
                                'BalanceNarrative', jsonb_build_array(
                                        jsonb_build_object(
                                                ...
                                                'BalanceNarrativeValue', 'Cash and Cash Alternatives'
                                        ),
                                        jsonb_build_object(
                                                'BalanceNarrativeType', 'AssetPercentage',
...
ALTER FUNCTION public.extract_summary_details_from_semi_bal_for_asset_alloc(text, text, text, text, text)
    OWNER TO "DBUser";
