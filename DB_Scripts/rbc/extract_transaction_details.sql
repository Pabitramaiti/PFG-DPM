-- FUNCTION: public.extract_transaction_details(text, text, text, text, text, text[], text[], text)

-- DROP FUNCTION IF EXISTS public.extract_transaction_details(text, text, text, text, text, text[], text[], text);

CREATE OR REPLACE FUNCTION public.extract_transaction_details(
	p_identifier_value text,
	clientid text,
	origin text,
	sectiontype text,
	transactioncategory_arg text,
	transactiontype_arg text[],
	transactionsubtype_arg text[],
	sorting_field text)
    RETURNS void
    LANGUAGE 'plpgsql'
    COST 100
...
ELSIF transactioncategory_arg = 'MonthlyTransactionConfirmation' AND ('Other' = ANY(transactiontype_arg)) THEN
                sectionheader='Monthly Transaction Confirmation Summary';
...
ELSIF transactioncategory_arg = 'BookkeepingJournalEntry' AND ('ChecksActivity' = ANY(transactiontype_arg)) THEN
                sectionheader='Checking Activity';
ELSIF transactioncategory_arg = 'BookkeepingJournalEntry' AND ('CardActivity' = ANY(transactiontype_arg)) THEN
                sectionheader='Debit Activity';
ELSIF transactioncategory_arg = 'BookkeepingJournalEntry' AND ('ChecksAndCardActivity' = ANY(transactiontype_arg)) THEN
...
ELSIF transactioncategory_arg = 'MonthlyDividendReinvestment' AND ('Other' = ANY(transactiontype_arg)) THEN
...
WITH detail_row_intermediate AS (
    ...
    FROM transactionparty tp
        ...
        LEFT JOIN transactionprice tprice ON tp.transactionid = tprice.transactionid
                AND tprice.transactionpricetype IN ('ExecutionPrice','OrderPrice','CurrentPrice','MarkUpDown')
        ...
        LEFT JOIN transactioninstrument tinst ON tp.transactionid = tinst.transactionid
                ...
                AND tinst.transactioninstrumentidentifiervalue <> 'N/A'
...
total_data_amount AS (
        ...
        SUM(
                CASE
                        WHEN transactionamounttype = 'CostBasis'
                        THEN transactionamountvalue
                        ELSE NULL
                END
        ) AS TotalNetCost,
        ...
        -- Total OtherDebit for Checking Activity
        SUM(
                CASE
        ...
        -- Total OtherCredit for Checking Activity
        SUM(
                CASE
        ...
        SELECT
                ...
                jsonb_path_query_array(
                                jsonb_build_array(
                                        ...
                                        ,
                                        CASE
                                                WHEN TotalNetCost IS NOT NULL
                                                THEN jsonb_strip_nulls(
                                                jsonb_build_object(
                                                        'TransactionAmountType', 'TotalNetCost',
                                                        'TransactionAmountValue', TotalNetCost,
                                                        'TransactionAmountCurrency', transactionamountcurrency
                                                )
                                        )
                                        END
...
ALTER FUNCTION public.extract_transaction_details(text, text, text, text, text, text[], text[], text)
    OWNER TO "DBUser";
