...
BEGIN
    ...
    SELECT 
        ...
        (regexp_replace(t1.partyorigin, '^[^_]+_([^_]+)_[^_]+_[^_]+\.json$', '\1') || '.json')::VARCHAR AS ma_partyorigin,
    ...
    WHERE (t1.partyid = ANY(party_ids) or t1.partyparentid = ANY(party_ids)) AND t1.partystatus = 'Active';
END;
