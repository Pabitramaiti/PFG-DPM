CREATE TABLE public.bios_socgen_out_recon (
    ...
    CONSTRAINT bios_socgen_out_recon_pkey PRIMARY KEY (filename)
);

CREATE INDEX index_bios_socgen_out_recon ON public.bios_socgen_out_recon USING btree (accountnumber, filename, status);

-- Permissions

ALTER TABLE public.bios_socgen_out_recon OWNER TO "DBUser";
GRANT ALL ON TABLE public.bios_socgen_out_recon TO "DBUser";



 CREATE TABLE public.bios_socgen_out_recon_data_account_list (
...
CREATE INDEX index_account_list ON public.bios_socgen_out_recon_data_account_list USING btree (accountnumber, clientid, status);

-- Permissions

ALTER TABLE public.bios_socgen_out_recon_data_account_list OWNER TO "DBUser";
GRANT ALL ON TABLE public.bios_socgen_out_recon_data_account_list TO "DBUser";

CREATE INDEX idx_recon_acc_status ON public.bios_socgen_out_recon (accountNumber, status); 
CREATE INDEX idx_recon_acc ON public.bios_socgen_out_recon (accountNumber); 
CREATE INDEX idx_data_acc_brx ON public.bios_socgen_out_recon_data_account_list (accountNumber, brxStatus); 
CREATE INDEX idx_data_partytype ON public.bios_socgen_out_recon_data_account_list (partyType);
CREATE INDEX idx_recon_error ON public.bios_socgen_out_recon (accountNumber) WHERE status = 'ERROR'; 
CREATE INDEX idx_recon_processed ON public.bios_socgen_out_recon (accountNumber) WHERE status = 'PROCESSED';

-------------------------------------------------------------------------------

CREATE OR REPLACE FUNCTION public.create_socgen_recon_tables()
RETURNS TEXT
LANGUAGE plpgsql
AS $$
BEGIN
    --------------------------------------------------------------------
    -- TABLE 1: bios_socgen_out_recon
    --------------------------------------------------------------------
    CREATE TABLE IF NOT EXISTS public.bios_socgen_out_recon (
        filename varchar(200) NOT NULL,
        accountnumber varchar(50) NOT NULL,
        clientid varchar(50),
        status varchar(50),
    ...
    CREATE INDEX IF NOT EXISTS index_bios_socgen_out_recon
        ON public.bios_socgen_out_recon (accountnumber, filename, status);

    ALTER TABLE public.bios_socgen_out_recon OWNER TO "DBUser";
    GRANT ALL ON TABLE public.bios_socgen_out_recon TO "DBUser";

    --------------------------------------------------------------------
    -- TABLE 2: bios_socgen_out_recon_data_account_list
    --------------------------------------------------------------------
    CREATE TABLE IF NOT EXISTS public.bios_socgen_out_recon_data_account_list (
    ...
    CREATE INDEX IF NOT EXISTS index_account_list
        ON public.bios_socgen_out_recon_data_account_list (accountnumber, clientid, status);

    ALTER TABLE public.bios_socgen_out_recon_data_account_list OWNER TO "DBUser";
    GRANT ALL ON TABLE public.bios_socgen_out_recon_data_account_list TO "DBUser";
RETURN 'Socgen Recon tables are created!';
END;
$$;

select public.create_socgen_recon_tables();


---------------------------------------------------------------------------------------------------------------
-- Drop FUNCTION
-- DROP FUNCTION socgen_statements_truncate_schema;

--✅ Step 1: Function to Truncate All Tables from selected schema with Optional Extra Exclusions
CREATE OR REPLACE FUNCTION public.socgen_statements_truncate_schema(p_client_name varchar, p_db_name varchar, p_exclude_tables varchar DEFAULT NULL)
RETURNS TEXT
LANGUAGE plpgsql
AS $$
DECLARE
    ...
    exclude_tables TEXT[] := ARRAY['brx_details','brx_db_log_details','databasechangelog', 'databasechangeloglock'];
...
BEGIN
    ...
    IF UPPER(p_client_name) = 'SOCGEN_STATEMENTS' AND UPPER(p_db_name) = 'SOCGEN' THEN
...
--select public.socgen_statements_truncate_schema('SOCGEN_STATEMENTS','SOCGEN','bios_socgen_out_recon_data_account_list, bios_socgen_out_recon');
-- SELECT public.socgen_statements_truncate_schema('SOCGEN_STATEMENTS','SOCGEN',null);
...
BEGIN
    WITH updated AS (
        UPDATE public.bios_socgen_out_recon_data_account_list AS anl
        SET 
        ...
        FROM public.bios_socgen_out_recon AS re
...
BEGIN
    ...
    FROM public.bios_socgen_out_recon re
    JOIN public.bios_socgen_out_recon_data_account_list anl
      ON anl.accountNumber = re.accountNumber
    WHERE (anl.brxStatus IS NULL OR anl.brxStatus = '')
      AND UPPER(anl.partyType) = 'INDIVIDUAL ACCOUNT'
      AND re.status = 'PROCESSED'
      AND NOT EXISTS (
            SELECT 1
            FROM public.bios_socgen_out_recon err
	...
	,
	updated_rows AS (
	    UPDATE public.bios_socgen_out_recon_data_account_list anl
...
BEGIN
	...
	--For PROCESSED Accounts
        ...
        WHERE (anl.brxStatus IS NULL OR anl.brxStatus = '')
          ...
          AND NOT EXISTS (
                SELECT 1
                FROM public.bios_socgen_out_recon sis
...
BEGIN
    ...
    WITH pending AS (
        ...
        FROM public.bios_socgen_out_recon_data_account_list
    ...
    master_status AS (
        ...
        FROM public.bios_socgen_out_recon r
    ...
    --------------------------------------------------------------------
    updated_rows AS (
        UPDATE public.bios_socgen_out_recon_data_account_list a
...
BEGIN
    WITH sister AS (
        ...
        FROM public.bios_socgen_out_recon_data_account_list anl
    ...
    sister_status AS (
        SELECT 
            ...
            COUNT(*) FILTER (WHERE r.status IS NULL OR r.status='') AS notprocessed
        FROM sister s
        JOIN public.bios_socgen_out_recon r 
