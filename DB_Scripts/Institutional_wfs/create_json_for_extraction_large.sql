        ...
        SummaryData AS (
            SELECT summary_object FROM (
                ...
                SELECT jsonb_build_object(
                    ...
                    'recordtype', 'Detail',
                    'thisperiod', td.AssetClassTotalBeginningValue::numeric,
                    'sectiontype', 'Summary',
                    'sectionheader', 'Money Market Mutual Funds Summary',
                    'classification', 'Opening Balance',
                    'transactionamountcurrency', currency_data
                ) AS summary_object
                FROM FilteredTotalData td
                WHERE td.sectionheader = 'Money Market Mutual Funds'
                -- AND td.AssetClassTotalBeginningValue != 0
                ...
                SELECT jsonb_build_object(
                    ...
                    'clientid', td.clientid,
                    'recordtype', 'Detail',
                    -- 'thisperiod', td.MMMFSummaryDepandAdditions::numeric,
                    'thisperiod', SUM(td.MMMFSummaryDepandAdditions)::numeric,
                    'sectiontype', 'Summary',
                    'sectionheader', 'Money Market Mutual Funds Summary',
                ...
                FROM FilteredTotalData td
                WHERE td.sectionheader = 'Money Market Mutual Fund Activity'
                -- AND td.MMMFSummaryDepandAdditions != 0
                GROUP BY td.partyid, td.clientid, currency_data
              ...
              UNION ALL
                ...
                SELECT jsonb_build_object(
                    ...
                    'clientid', td.clientid,
                    'recordtype', 'Detail',
                    'thisperiod', SUM(td.MMMFSummaryDistandSubtractions)::numeric,
                    'sectiontype', 'Summary',
                    'sectionheader', 'Money Market Mutual Funds Summary',
                ...
                FROM FilteredTotalData td
                WHERE td.sectionheader = 'Money Market Mutual Fund Activity'
                -- AND td.MMMFSummaryDistandSubtractions != 0
              GROUP BY td.partyid, td.clientid, currency_data
              ...
              UNION ALL
                ...
                SELECT jsonb_build_object(
                    ...
                    'clientid', td.clientid,
                    'recordtype', 'Detail',
                    'thisperiod', SUM(td.MMMFSummaryDivReinvested)::numeric,
                    'sectiontype', 'Summary',
                    'sectionheader', 'Money Market Mutual Funds Summary',
                ...
                FROM FilteredTotalData td
                WHERE td.sectionheader = 'Money Market Mutual Fund Activity'
                -- AND td.MMMFSummaryDivReinvested != 0
                GROUP BY td.partyid, td.clientid, currency_data

                UNION ALL

                -- 5. Change in Value - calculated here by aggregating FilteredTotalData per client & currency
                SELECT jsonb_build_object(
                    'partyid', agg.partyid,
                    ...
                    'thisperiod', 
                        agg.closing_balance - (
                            agg.opening_balance + agg.deposits_and_additions + agg.distributions_and_subtractions + agg.dividends_reinvested
                        ),
                    'transactionamountcurrency', currency_data
                ) AS summary_object
                FROM (
                    SELECT
                        clientid,
                        partyid,
                        COALESCE(MAX(CASE WHEN sectionheader = 'Money Market Mutual Funds' THEN AssetClassTotalValue END), 0) AS closing_balance,
                        COALESCE(MAX(CASE WHEN sectionheader = 'Money Market Mutual Funds' THEN AssetClassTotalBeginningValue END), 0) AS opening_balance,
                        --COALESCE(MAX(CASE WHEN sectionheader = 'Money Market Mutual Fund Activity' THEN MMMFSummaryDepandAdditions END), 0) AS deposits_and_additions,
                        --COALESCE(MAX(CASE WHEN sectionheader = 'Money Market Mutual Fund Activity' THEN MMMFSummaryDistandSubtractions END), 0) AS distributions_and_subtractions,
                        --COALESCE(MAX(CASE WHEN sectionheader = 'Money Market Mutual Fund Activity' THEN MMMFSummaryDivReinvested END), 0) AS dividends_reinvested
                  
                        -- SUMs for activity rows (these come from multiple rows per party/client)
                        COALESCE(SUM(CASE WHEN sectionheader = 'Money Market Mutual Fund Activity' THEN MMMFSummaryDepandAdditions END), 0) AS deposits_and_additions,
                        COALESCE(SUM(CASE WHEN sectionheader = 'Money Market Mutual Fund Activity' THEN MMMFSummaryDistandSubtractions END), 0) AS distributions_and_subtractions,
                        COALESCE(SUM(CASE WHEN sectionheader = 'Money Market Mutual Fund Activity' THEN MMMFSummaryDivReinvested END), 0) AS dividends_reinvested,

                        BOOL_OR(sectionheader = 'Money Market Mutual Funds') AS has_mmf_summary,
                        BOOL_OR(sectionheader = 'Money Market Mutual Fund Activity') AS has_mmf_activity
                  
                    FROM FilteredTotalData
                    GROUP BY partyid, clientid
                ) agg
                
                WHERE
                    -- agg.closing_balance != 0 OR
                    -- agg.opening_balance != 0 OR
                    -- agg.deposits_and_additions != 0 OR
                    -- agg.distributions_and_subtractions != 0 OR
                    -- agg.dividends_reinvested != 0
                    (agg.has_mmf_summary OR agg.has_mmf_activity)
                ...
                SELECT jsonb_build_object(
                    ...
                    'clientid', td.clientid,
                    'recordtype', 'Total',
                    'thisperiod', td.AssetClassTotalValue::numeric,
                    'sectiontype', 'Summary',
                    'sectionheader', 'Money Market Mutual Funds Summary',
