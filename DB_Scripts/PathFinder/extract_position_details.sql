CREATE OR REPLACE FUNCTION public.extract_position_details(p_identifier_value text, clientid text, origin text, sectiontype text,
sectionheader text)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
BEGIN
    WITH detail_row AS (
		...
		AND instiden.instrumentidentifiertype IN ('CUSIP','Symbol')
            ...
            SUM(
                CASE
                    WHEN positionamounttype = 'CostBasis'
						 AND instrumentassetclassvalue NOT IN ('Mutual Funds','Alternative Investments')
