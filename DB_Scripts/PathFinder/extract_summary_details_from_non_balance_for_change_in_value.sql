CREATE OR REPLACE FUNCTION public.extract_summary_details_from_non_balance_for_change_in_value(p_identifier_value text, client_data text, origin text, sectiontype text, sectionheader text)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
	...
	-- insert in main summary temp table
	...
	--	('NetChangeAmount', v_net_change_amount, NULL, '1', 'Detail'),
		('AssetsNotHeldValue', v_asset_not_held_value, 'Assets Not Held at Hexatone Investments Services*', '1', 'Detail'),
		('TotalBalance', v_total_balance, 'Total', '1', 'Detail');
		
	-- insert in main summary temp table ( for YTD )
		insert into temp_extraction_results_change_in_value ( amounttype, amountvalue, narrative, isptd,  recordtype)
		values
		('AssetsNotHeldValue', v_asset_not_held_value, 'Assets Not Held at Hexatone Investments Services*', '0', 'Detail'),
