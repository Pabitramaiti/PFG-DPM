CREATE OR REPLACE FUNCTION public.extract_summary_details_from_non_balance_for_gainloss(p_identifier_value text, client_data text, origin text, sectiontype text, sectionheader text)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
	...
	-- Getting realized data from Transactions
		...
		FOR db_rec IN EXECUTE dynamic_sql using p_identifier_value, client_data, origin, currency_data, 'Detail'
			LOOP

			FOR amount_index IN 0..COALESCE(jsonb_array_length(db_rec.data -> 'PositionAmount'),0)-1  LOOP
				IF db_rec.data -> 'PositionAmount' -> amount_index ->> 'PositionAmountType' = 'UnrealizedGainLoss' THEN

					FOR narrative_index IN 0..COALESCE(jsonb_array_length(db_rec.data -> 'PositionNarrative'),0)-1  LOOP
						IF db_rec.data -> 'PositionNarrative' -> narrative_index ->> 'PositionNarrativeType' = 'Term'
						   AND db_rec.data -> 'PositionNarrative' -> narrative_index ->> 'PositionNarrativeValue' = 'Short' THEN

							temp_amnt := (db_rec.data -> 'PositionAmount' -> amount_index ->> 'PositionAmountValue')::numeric;
							IF	temp_amnt >= 0 THEN
								v_unrealized_ytd_short_term_gain := v_unrealized_ytd_short_term_gain + temp_amnt;
							ELSE
								v_unrealized_ytd_short_term_loss := v_unrealized_ytd_short_term_loss + temp_amnt;
							END IF;
							v_unrealized_ytd_short_term_gain_or_loss := v_unrealized_ytd_short_term_gain_or_loss + temp_amnt;
							v_unrealized_ytd_total_gain_or_loss := v_unrealized_ytd_total_gain_or_loss + temp_amnt;
						END IF;
						IF db_rec.data -> 'PositionNarrative' -> narrative_index ->> 'PositionNarrativeType' = 'Term'
						   AND db_rec.data -> 'PositionNarrative' -> narrative_index ->> 'PositionNarrativeValue' = 'Long' THEN

							temp_amnt := (db_rec.data -> 'PositionAmount' -> amount_index ->> 'PositionAmountValue');
							IF	temp_amnt >= 0 THEN
								v_unrealized_ytd_long_term_gain := v_unrealized_ytd_long_term_gain + temp_amnt;
							ELSE
								v_unrealized_ytd_long_term_loss := v_unrealized_ytd_long_term_loss + temp_amnt;
							END IF;
							v_unrealized_ytd_long_term_gain_or_loss := v_unrealized_ytd_long_term_gain_or_loss + temp_amnt;
							v_unrealized_ytd_total_gain_or_loss := v_unrealized_ytd_total_gain_or_loss + temp_amnt;
						END IF;
					END LOOP;
				END IF;
		...
		--	RAISE NOTICE 'v_realized_ptd_short_term_gain_or_loss: %', v_realized_ptd_short_term_gain_or_loss;
		--	RAISE NOTICE 'v_realized_ytd_short_term_gain_or_loss: %', v_realized_ytd_short_term_gain_or_loss;
		--	RAISE NOTICE 'v_unrealized_ytd_short_term_gain_or_loss: %', v_unrealized_ytd_short_term_gain_or_loss;
