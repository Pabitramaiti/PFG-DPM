...
def copy_object(bucket_name, source_files, destination_key):
    ...
    # Ensure the destination_key ends with a slash
    if not destination_key.endswith('/'):
        destination_key = f"{destination_key}/"
...
def lambda_handler(event, context):
    try:
        ...
        if event is not None:
            ...
            print(f"Received event body: {json.dumps(body, indent=2)}")
        else:
            raise ValueError("body parameter is empty")

        # Extract parameters
        s3_operation_type = body.get('OperationType')
